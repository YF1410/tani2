...
PlayerObject::PlayerObject(XMFLOAT3 startPos, bool isAfterimage) :
	GameObjCommon(
...
void PlayerObject::Update() {
	...
	if (!isAfterimage) {
		...
		if (attack.is && velocity.Length() < 105 && !isHitStop) {
			attack.is = false;
			isBounce = false;
			animationType = MOVE;
			animationChangeFrag = true;
		...
		PlayerAfterimage::playerAfterimages.push_back(new PlayerAfterimage(*this->objectData.get()));
